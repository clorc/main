...
fn generate_2d_toeplitz(matrix: DynamicMatrix, ncols: usize, nrows:usize, image_vector_rows: usize) {
    ...
    for i in 0..=matrix.nrows()-1 {
        toeplitz_vector.push(generate_1d_toeplitz(matrix.row(i).transpose(), ncols));
    }

    // initializing the block matrix
    let mut block_toeplitz: DynamicMatrix = base::Matrix::zeros_generic(Dyn(toeplitz_vector.len()*nrows), Dyn(image_vector_rows));
